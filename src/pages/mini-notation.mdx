...
Mini-notation, when parsed, turns into a PureScript data structure called a `Cycle`. To create algorithmic and pattern-based music, its often useful to work directly with `Cycle`-s instead of working with `String`-s. Let's see how that's done!
