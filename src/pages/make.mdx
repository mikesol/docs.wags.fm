...
We'll get into what the tilde `~` and what the square brackets `[]` mean in the next section. For now, the important bit to note is that there are six different events spread over the six voices and each one starts with the function `s`.
...
`sounds` are key/value pairs of names of sounds that you'll use in your session and their corresponding URL. The type is `Object BufferURL`, and while there are a few ways to create one of these, the easiest is by using `fromHomogeneous` and `hmap BufferUrl`. You can copy and paste that snippet below and use it wherever you need to use sounds.
